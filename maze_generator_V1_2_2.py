...
class entrance_UI(PyQt5_QDialog):
    ...
    def setupUI(self):  #设置窗口
        ...
        self.setBackground('背景.png')
        self.setWindowTitle('迷宫生成器 V1.2.2')           #设置窗口标题

        self.name_dialog = PyQt5_Qlabel(self,self.dialog_size_long/4-25,30,600,200)
        self.name_dialog.setFont(QFont('宋体',20))
        self.name_dialog.setTextColor('white')
        ...
        self.enter_buttom_9_9.setPressedBackground('simple_buttom_#03A89E_暗.png')  #设置按钮被按压时的背景图片
        ...
        self.enter_buttom_13_13.setPressedBackground('simple_buttom_#03A89E_暗.png')  
    ...
    def map_dispose_and_show(self,size):
        if size == 9:
            from maze_generator_9time9_dispose import Compute_9time9,UI_show_map,answer
            Dispose = Compute_9time9()
            Dispose.dispose()
        if size == 13:
            from maze_generator_13time13_dispose import Compute_13time13,UI_show_map,answer
